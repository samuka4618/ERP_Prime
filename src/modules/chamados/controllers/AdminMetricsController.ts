...
import { ActivityTrackingService } from '../../../core/system/ActivityTrackingService';
import { tokenCacheService } from '../../../core/auth/TokenCacheService';
...
export class AdminMetricsController {
  ...
  static async getUserMetrics(req: Request, res: Response): Promise<void> {
    try {
      ...
      const userMetrics = dbUserMetrics.map((user: any) => {
        const onlineUser = onlineUsers.find((online: any) => online.userId === user.user_id);
        return {
          ...user,
  ...
  static async getAttendantMetrics(req: Request, res: Response): Promise<void> {
    try {
      ...
      const attendantMetrics = dbAttendantMetrics.map((attendant: any) => {
        const onlineAttendant = onlineAttendants.find((online: any) => online.userId === attendant.attendant_id);
        return {
          ...attendant,
  ...
  static async getAttendantPerformanceReport(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (startDate && endDate) {
        ...
        filteredMetrics = attendantMetrics.filter((metric: any) => {
      ...
      // Calcular rankings
      const sortedByTickets = [...filteredMetrics].sort((a: any, b: any) => b.total_tickets_this_month - a.total_tickets_this_month);
      const sortedByResolution = [...filteredMetrics].sort((a: any, b: any) => a.avg_resolution_time - b.avg_resolution_time);
      const sortedBySatisfaction = [...filteredMetrics].sort((a: any, b: any) => b.customer_satisfaction - a.customer_satisfaction);

      const report = {
        summary: {
          total_attendants: filteredMetrics.length,
          online_attendants: filteredMetrics.filter((a: any) => a.is_online).length,
          total_tickets_this_month: filteredMetrics.reduce((sum: number, a: any) => sum + a.total_tickets_this_month, 0),
          avg_resolution_time: Math.round(filteredMetrics.reduce((sum: number, a: any) => sum + a.avg_resolution_time, 0) / filteredMetrics.length || 0),
          avg_satisfaction: Math.round(filteredMetrics.reduce((sum: number, a: any) => sum + a.customer_satisfaction, 0) / filteredMetrics.length || 0)
        },
        rankings: {
