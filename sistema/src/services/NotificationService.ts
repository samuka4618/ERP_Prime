...
import { Notification, TicketStatus } from '../types';
...
export class NotificationService {
  ...
  static async createNotification(
    ...
    return await NotificationModel.create(userId, ticketId, type, title, message);
  }
  ...
  static async createClientRegistrationNotification(
    ...
    try {
      ...
      await NotificationModel.create(userId, 0, type as any, title, message);
    } catch (error) {
  ...
  static async sendEmail(to: string, subject: string, text: string): Promise<void> {
    ...
    try {
      await this.transporter.sendMail({
        ...
        to,
        subject: `[Sistema de Chamados] ${subject}`,
        text
      });
