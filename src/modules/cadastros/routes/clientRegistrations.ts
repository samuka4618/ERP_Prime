...
import { uploadClientImages, uploadClientImagesOptional } from '../../../shared/middleware/uploadClientImages';
import * as dotenv from 'dotenv';
import { DatabaseService } from '../../../../tools/cadastros-legacy/src/services/databaseService';
import { AtakService } from '../../../../tools/cadastros-legacy/src/services/atakService';
import { isAtakConfigured } from '../../../../tools/cadastros-legacy/src/services/atakAuth';
...
router.get('/', requirePermission('registrations.view'), async (req, res) => {
  try {
    const filters = {
...
router.post('/', requirePermission('registrations.create'), uploadClientImages, async (req, res): Promise<void> => {
  try {
...
router.put('/:id', requirePermission('registrations.edit'), uploadClientImagesOptional, async (req, res): Promise<void> => {
  try {
...
router.get('/recent-history', requirePermission('registrations.history.view'), authorize(UserRole.ADMIN), async (req, res) => {
  try {
...
router.get('/:id', requirePermission('registrations.view'), async (req, res) => {
  try {
    const id = parseInt(req.params.id);
...
router.put('/:id/status', requirePermission('registrations.approve'), authorize(UserRole.ADMIN), async (req, res) => {
  try {
    const id = parseInt(req.params.id);
...
router.put('/:id/financial', requirePermission('registrations.financial.edit'), authorize(UserRole.ADMIN, UserRole.ATTENDANT), async (req, res) => {
  try {
    const id = parseInt(req.params.id);
...
router.get('/:id/atak', requirePermission('registrations.analise_credito.view'), authorize(UserRole.ADMIN, UserRole.ATTENDANT), async (req, res) => {
  try {
    const id = parseInt(req.params.id);
    ...
    // Verificar se o Atak está configurado (token + base URL, ou username + password + base URL)
    if (!isAtakConfigured()) {
      res.status(400).json({
        success: false,
        message: 'Configurações do Atak não encontradas. No .env use ATAK_BASE_URL e (ATAK_TOKEN ou ATAK_USERNAME+ATAK_PASSWORD).'
      });
      return;
    }
...
router.post('/:id/reprocess', requirePermission('registrations.edit'), authorize(UserRole.ADMIN), async (req, res) => {
  try {
    const id = parseInt(req.params.id);
