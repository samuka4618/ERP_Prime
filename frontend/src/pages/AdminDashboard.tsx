...
const AdminDashboard: React.FC = () => {
  const { hasPermission, loadingPermissions, permissions } = usePermissions();
  ...
  useEffect(() => {
    if (loadingPermissions) {
      console.log('[AdminDashboard] ⏳ Aguardando carregamento de permissões...');
      return;
    }

    const canAccess = hasPermission('admin.dashboard.view');
    const perm24 = permissions.find((p: any) => p.code === 'admin.dashboard.view');
    
    console.log('[AdminDashboard] ⚠️ Verificando permissão admin.dashboard.view:', {
      canAccess,
      loadingPermissions,
    ...
    if (!canAccess) {
      console.log('[AdminDashboard] ⚠️ Acesso negado, redirecionando...');
      setAccessDenied(true);
      toast.error('Você não tem permissão para acessar a página de Monitoramento.');
      setTimeout(() => {
    ...
    console.log('[AdminDashboard] ✅ Acesso permitido, carregando dados...');
    fetchData();
  ...
  }, [loadingPermissions, hasPermission, permissions, navigate, fetchData, trackUserActivity]);

  // Renderizar loading ou acesso negado antes de qualquer coisa
  if (loadingPermissions) {
    return <LoadingSpinner />;
  }

  if (accessDenied || (!loadingPermissions && !hasPermission('admin.dashboard.view'))) {
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-4">Acesso Negado</h1>
          <p className="text-gray-600 dark:text-gray-400">
            Você não tem permissão para acessar a página de Monitoramento.
          </p>
    ...
    <div className="p-6">
      <div className="mb-8">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Monitoramento</h1>
      ...
      {activeTab === 'overview' && overview && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-6">
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow">
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  <p className="text-sm font-medium text-gray-600 dark:text-gray-400">Solicitações de Compra</p>
                  <p className="text-2xl font-bold text-gray-900 dark:text-white">{overview.total_solicitacoes}</p>
                  <p className="text-sm text-gray-600 dark:text-gray-400">
                    {overview.solicitacoes_criadas_hoje} criadas hoje
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow">
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  <p className="text-sm font-medium text-gray-600 dark:text-gray-400">Orçamentos</p>
                  <p className="text-2xl font-bold text-gray-900 dark:text-white">{overview.total_orcamentos}</p>
                  <p className="text-sm text-gray-600 dark:text-gray-400">
                    {overview.orcamentos_pendentes} pendentes
                  </p>
                </div>
              </div>
            </div>
              ...
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  ...
                  <p className="text-2xl font-bold text-gray-900 dark:text-white">
                    R$ {overview.valor_total_solicitacoes.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </p>
                  <p className="text-sm text-gray-600 dark:text-gray-400">
                    {overview.solicitacoes_aprovadas} aprovadas
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Status de Compras</h3>
              <div className="space-y-4">
                <div className="flex justify-between">
                  <span className="text-gray-600 dark:text-gray-400">Pendentes Aprovação</span>
                  <span className="font-semibold text-yellow-600">{overview.solicitacoes_pendentes}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600 dark:text-gray-400">Em Cotação</span>
                  <span className="font-semibold text-blue-600">{overview.solicitacoes_em_cotacao}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600 dark:text-gray-400">Aprovadas</span>
                  <span className="font-semibold text-green-600">{overview.solicitacoes_aprovadas}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600 dark:text-gray-400">Orçamentos Pendentes</span>
                  <span className="font-semibold text-orange-600">{overview.orcamentos_pendentes}</span>
                </div>
              </div>
            </div>
          </div>
