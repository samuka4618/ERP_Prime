...
| **"A API retornou uma página em vez de dados"** / rotas como `/api/users/entra/list` ou `/api/auth/providers` devolvem HTML | Proxy ou CDN (Cloudflare, nginx, etc.) na frente do backend está servindo o `index.html` do frontend para rotas `/api/*`, ou o timeout do proxy é menor que o tempo de resposta do backend. | **Configurar o proxy** para: (1) encaminhar **todas** as requisições cujo path comece com `/api` ao backend Node (não servir SPA para essas rotas); (2) **não** usar "fallback para index.html" em paths `/api/*`; (3) aumentar o **timeout** do upstream (ex.: 30–60 s) se o backend demorar (ex.: chamadas ao Microsoft Graph). No **Railway**, se usar domínio customizado (ex.: `api.seudominio.com`), garantir que o tráfego vá para o serviço Node e não para um front estático. Após alterar o proxy, fazer redeploy/restart e testar de novo.                                                                                |
