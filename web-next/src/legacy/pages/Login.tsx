  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error?.response?.status === 409 && code === 'SESSION_CONFLICT') {
        const n = error?.response?.data?.activeSessions;
        const detail =
          typeof n === 'number'
            ? ` Existem ${n} sessão(ões) com refresh ainda válido.`
            : '';
        const ok = window.confirm(
          `${msg}${detail}\n\nDeseja continuar e encerrar a(s) outra(s) sessão(ões)?`
        );
        if (ok) {
          try {
            await login({ ...formData, forceDisconnectOthers: true });
            toast.success('Login realizado; outras sessões foram encerradas.');
          } catch (e2: any) {
            toast.error(e2?.response?.data?.error || e2?.response?.data?.message || 'Erro ao fazer login');
          }
        }
      } else {
        toast.error(msg);
