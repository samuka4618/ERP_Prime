...
class ApiService {
  ...
  constructor() {
    ...
    if (isElectron) {
      // No Electron, usar URL do backend configurada
      // Se ainda não estiver disponível, usar fallback
      const electronBackendUrl = (window as any).__ELECTRON_BACKEND_URL__ || 'http://192.168.14.143:3000';
      baseURL = `${electronBackendUrl}/api`;
      console.log('API Base URL configurada para Electron:', baseURL);
    } else {
      // Detectar automaticamente a URL base baseada no hostname atual (navegador web)
      ...
      if (hostname === 'localhost' || hostname === '127.0.0.1') {
        ...
        // Para IPs da rede, usar IP completo com porta 3000
        baseURL = `${window.location.protocol}//${hostname}:3000/api`;
      }
