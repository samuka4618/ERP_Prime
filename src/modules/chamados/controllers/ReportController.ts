...
import { ReportType, ReportParameters, ReportStatus, Report } from '../types';
...
export class ReportController {
  ...
  static async createReport(req: Request, res: Response) {
    try {
      ...
      if (reportData.type === 'custom' && reportData.custom_fields) {
        try {
          const customFields = typeof reportData.custom_fields === 'string'
            ? JSON.parse(reportData.custom_fields)
            : reportData.custom_fields;
          const fields = customFields?.fields;
          if (Array.isArray(fields) && fields.length > 0) {
            const validation = CustomReportService.validateNoMixedGraphs(fields);
            if (!validation.valid) {
              res.status(400).json({
                success: false,
          ...
          res.status(400).json({
            success: false,
            message: 'Configuração do relatório personalizado (custom_fields) inválida.'
          });
          return;
        }
      }
  ...
  static async getReports(req: Request, res: Response): Promise<void> {
    try {
      ...
      logger.info('Relatórios encontrados', { 
        ...
        total: reports.total,
        reports: reports.data.map((r: Report) => ({ id: r.id, name: r.name, type: r.type }))
      }, 'REPORTS');
  ...
  static async updateReport(req: Request, res: Response): Promise<void> {
    try {
      ...
      if ((updateData.type === 'custom' || updateData.custom_fields) && updateData.custom_fields) {
        try {
          const customFields = typeof updateData.custom_fields === 'string'
            ? JSON.parse(updateData.custom_fields)
  ...
  // Método para processar agendamentos (chamado periodicamente pelo servidor)
  static async processScheduledReports(): Promise<void> {
    try {
      ...
      for (const schedule of dueSchedules) {
        const executedBy = (schedule.report as any)?.created_by || 1;
        let execution: { id: number } | null = null;

        try {
          if (!schedule.report) continue;

          const report = schedule.report as any;
          const parameters = report.parameters ? (typeof report.parameters === 'string' ? JSON.parse(report.parameters) : report.parameters) : {};

          execution = await ReportModel.createExecution({
          ...
          if (report.type === 'custom') {
            if (!report.custom_fields) {
              throw new Error('Relatório personalizado sem custom_fields');
            }
            const customFields = typeof report.custom_fields === 'string' ? JSON.parse(report.custom_fields) : report.custom_fields;
            reportData = await CustomReportService.executeCustomReport(customFields);
          } else {
            reportData = await ReportService.generateReport(report.type, parameters);
          }
          ...
          // Envio por email (quando implementado): await EmailService.sendReport(schedule.recipients, reportData, schedule.name);
          await ReportModel.updateScheduleNextExecution(schedule.id);
          logger.info(`Relatório agendado executado: ${schedule.name} (ID: ${schedule.id}), execução: ${execution.id}`);
        } catch (error) {
          const errorMessage = error instanceof Error ? error.message : 'Erro desconhecido';
  ...
  /** Validar campos do relatório personalizado (ex.: mistura Chamados + Compras) antes de salvar */
  static async validateCustomReportFields(req: Request, res: Response): Promise<void> {
    try {
      const body = req.body as { fields?: { table: string; name?: string; column?: string }[]; custom_fields?: string | object };
      let fields = body.fields;
      if (!fields && body.custom_fields != null) {
      ...
      const validation = CustomReportService.validateNoMixedGraphs(fields as any);
      res.status(200).json({
        success: true,
      ...
      res.status(400).json({
        success: false,
        valid: false,
        message: 'Payload inválido. Envie { fields: [...] } ou { custom_fields: { fields: [...] } }.'
      });
    }
  }
  ...
  static async createCustomReport(req: Request, res: Response): Promise<void> {
    try {
      ...
      // Impedir mistura de Chamados + Compras no mesmo relatório
      const fields = customFields?.fields;
      if (Array.isArray(fields) && fields.length > 0) {
        const mixValidation = CustomReportService.validateNoMixedGraphs(fields);
        if (!mixValidation.valid) {
          res.status(400).json({
            success: false,
            message: mixValidation.message
          });
          return;
        }
      }
  ...
  private static formatReportForCSV(data: any, executionId: number): string {
    ...
    lines.push(`Sistema: ERP PRIME v2.0`);
    lines.push('');
    ...
    lines.push('');
    lines.push('='.repeat(80));
    lines.push('RELATÓRIO GERADO AUTOMATICAMENTE PELO ERP PRIME');
