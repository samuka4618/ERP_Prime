/**
 * Backend SQLite para o ERP Prime (comportamento padrão).
 * Usado quando USE_POSTGRES não está definido ou é false.
 */
...
async function runSchemaMigrations(): Promise<void> {
  try {
    ...
    if (!agendamentosDescarga) {
      ...
      console.log('Migração: tabelas agendamentos_descarga criadas');
    }
    ...
    if (!hasDischargeStarted) {
      ...
      console.log('Migração: coluna discharge_started_at adicionada');
    }
    ...
    if (smsCount && (smsCount as { count: number }).count === 0) {
      await dbRun(`
        ...
        ('Chamado para doca (padrão)', '{{fornecedor_name}}: {{driver_name}}, você foi chamado para descarregamento. Data: {{scheduled_date}} {{scheduled_time}}. Doca: {{dock}}. Código: {{tracking_code}}.', 'arrival', 1),
        ('Liberação (padrão)', '{{fornecedor_name}}: {{driver_name}}, você foi liberado. Código: {{tracking_code}}. Obrigado!', 'release', 1)
      `);
      console.log('Migração: templates SMS padrão inseridos');
    }
    ...
    `);
    await dbRun(`INSERT OR IGNORE INTO role_permissions (role, permission_id, granted) SELECT 'admin', id, 1 FROM permissions WHERE module = 'descarregamento'`);
    await dbRun(`INSERT OR IGNORE INTO role_permissions (role, permission_id, granted) SELECT 'attendant', id, 1 FROM permissions WHERE code IN ('descarregamento.agendamentos.view', 'descarregamento.agendamentos.create', 'descarregamento.agendamentos.edit', 'descarregamento.fornecedores.view', 'descarregamento.fornecedores.create', 'descarregamento.fornecedores.edit', 'descarregamento.docas.view', 'descarregamento.docas.manage', 'descarregamento.formularios.manage', 'descarregamento.form_responses.view', 'descarregamento.form_responses.release', 'descarregamento.motoristas.view', 'descarregamento.formularios.view_responses', 'descarregamento.motoristas.liberar')`);
  } catch (err) {
    console.warn('Migração de schema:', (err as Error).message);
  }
...
export const executeSchema = async (): Promise<void> => {
  try {
    ...
    for (const line of lines) {
      ...
      if (trimmedLine.toUpperCase().includes('CREATE TRIGGER')) inTrigger = true;
    ...
    for (const statement of statements) {
      if (statement.trim()) {
        try {
          ...
          console.error('Erro ao executar statement:', statement.slice(0, 80));
          throw error;
        }
      }
    }
