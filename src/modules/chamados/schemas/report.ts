...
export const reportSchema = {
  create: Joi.object({
    ...
    custom_fields: Joi.alternatives().try(Joi.string(), Joi.object({ fields: Joi.array().items(Joi.object()) })).optional().messages({
      'alternatives.match': 'custom_fields deve ser string JSON ou objeto com fields'
    }),
  ...
  update: Joi.object({
    ...
    custom_fields: Joi.alternatives().try(Joi.string(), Joi.object({ fields: Joi.array().items(Joi.object()) })).optional().allow(null).messages({
      'alternatives.match': 'custom_fields deve ser string JSON ou objeto com fields'
    }),
