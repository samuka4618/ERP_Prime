...
export class SystemConfigModel {
  /**
   * Obtém as configurações GLOBAIS do sistema.
   * IMPORTANTE: Estas configurações são aplicadas para TODOS os usuários do sistema.
   * Não há configurações por usuário - todas são compartilhadas globalmente.
   */
    ...
    for (const item of configs) {
      switch (item.key) {
        ...
        case 'system_name':
          ...
          break;
        case 'system_subtitle':
          config.system_subtitle = item.value;
          break;
        case 'system_logo':
          config.system_logo = item.value;
          break;
        case 'system_version':
   ...
   * IMPORTANTE: As mudanças são aplicadas para TODOS os usuários do sistema.
   * Não há configurações por usuário - todas são compartilhadas globalmente.
   */
  static async updateSystemConfig(updates: Partial<SystemConfig>): Promise<SystemConfig> {
    ...
    if (updates.system_subtitle !== undefined) {
      updateMap['system_subtitle'] = updates.system_subtitle;
    }
    if (updates.system_logo !== undefined) {
      updateMap['system_logo'] = updates.system_logo;
    }
