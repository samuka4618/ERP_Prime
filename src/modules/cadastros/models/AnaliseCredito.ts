    ...
    try {
      ...
      try {
        [enderecoResult, dadosContatoResult, ocorrenciasResult, scoreResult, historicoResult, scrResult, sociosResult, quadroResult, consultasRealizadasResult] = await Promise.all([
          pool.request().query(`SELECT TOP 1 * FROM endereco WHERE id_empresa = ${empresaId} ORDER BY updated_at DESC, id DESC`).catch((err: any) => { console.warn('⚠️ Erro ao buscar endereço:', err); return { recordset: [] }; }),
          pool.request().query(`SELECT TOP 1 * FROM dados_contato WHERE id_empresa = ${empresaId} ORDER BY updated_at DESC, id DESC`).catch((err: any) => { console.warn('⚠️ Erro ao buscar dados_contato:', err); return { recordset: [] }; }),
          pool.request().query(`SELECT TOP 1 * FROM ocorrencias WHERE id_empresa = ${empresaId} ORDER BY updated_at DESC, id DESC`).catch((err: any) => { console.warn('⚠️ Erro ao buscar ocorrencias:', err); return { recordset: [] }; }),
          pool.request().query(`SELECT TOP 1 * FROM score_credito WHERE id_empresa = ${empresaId} ORDER BY updated_at DESC, id DESC`).catch((err: any) => { console.warn('⚠️ Erro ao buscar score_credito:', err); return { recordset: [] }; }),
          pool.request().query(`SELECT TOP 1 * FROM historico_pagamento_positivo WHERE id_empresa = ${empresaId} ORDER BY updated_at DESC, id DESC`).catch((err: any) => { console.warn('⚠️ Erro ao buscar historico_pagamento_positivo:', err); return { recordset: [] }; }),
          pool.request().query(`SELECT TOP 1 * FROM scr WHERE id_empresa = ${empresaId} ORDER BY updated_at DESC, id DESC`).catch((err: any) => { console.warn('⚠️ Erro ao buscar scr:', err); return { recordset: [] }; }),
          pool.request().query(`SELECT cpf, nome, entrada, participacao, valor_participacao, percentual_participacao, cargo FROM socios WHERE id_empresa = ${empresaId} ORDER BY id`).catch((err: any) => { console.warn('⚠️ Erro ao buscar socios:', err); return { recordset: [] }; }),
          pool.request().query(`SELECT cpf, nome, cargo, eleito_em FROM quadro_administrativo WHERE id_empresa = ${empresaId} ORDER BY id`).catch((err: any) => { console.warn('⚠️ Erro ao buscar quadro_administrativo:', err); return { recordset: [] }; }),
          pool.request().query(`SELECT * FROM consultas_realizadas WHERE id_empresa = ${empresaId} ORDER BY data_hora DESC`).catch((err: any) => { console.warn('⚠️ Erro ao buscar consultas_realizadas:', err); return { recordset: [] }; })
        ]);
      } catch (error) {
