...
export class ClientRegistrationModel {
  ...
  static async getStatistics(startDate?: string, endDate?: string): Promise<any> {
    ...
    return {
      ...
      // Se falhar ao conectar ao SQL Server, retornar estatísticas vazias
      console.warn('⚠️ [CLIENT-REGISTRATION-MODEL] SQL Server não disponível, retornando estatísticas vazias:', error instanceof Error ? error.message : 'Erro desconhecido');
      ...
      return {
        ...
        approvedCount: 0,
        completedCount: 0,
        warning: 'SQL Server não disponível. Estatísticas não podem ser calculadas.'
      };
    }
  }
  ...
  static async updateStatus(
    ...
    return await executeSqlTransaction(async (transaction: sql.Transaction) => {
